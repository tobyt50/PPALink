...
export async function deleteApplication(applicationId: string, agencyId: string, actorId: string) {
  ...
  // Use a transaction to handle related records first, then the application
  const result = await prisma.$transaction(async (tx) => {
    // Delete related Offers (cascades implicitly, but explicit for control)
    await tx.offer.deleteMany({
      where: { applicationId: applicationId },
    });
    
    // Delete related Interviews (cascades implicitly, but explicit for control)
