...
const OnboardingGuard = ({ children }: { children: React.ReactNode }) => {
  ...
  if (
    ...
    candidateProfile &&
    !candidateProfile.hasCompletedOnboarding
  ) {
    if (!location.pathname.startsWith("/onboarding/candidate")) {
      if (candidateProfile.profileType === "PROFESSIONAL") {
        return <Navigate to="/onboarding/candidate/skills" replace />;
      } else {
  ...
  if (
    ...
    !currentUser.hasCompletedAdminOnboarding
  ) {
