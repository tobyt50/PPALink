    ...
    <>
      ...
      <div className="rounded-2xl bg-white dark:bg-zinc-900 shadow-md dark:shadow-none dark:ring-1 dark:ring-white/10 ring-1 ring-gray-100 overflow-hidden">
        ...
        <div className="p-5 border-b border-gray-100 dark:border-zinc-800 flex items-center justify-between">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-zinc-50">Work Experience</h2>
          {isOwner && (
            <Button
              variant="outline"
              size="sm"
        ...
        <div className="p-6">
          {experiences && experiences.length > 0 ? (
              ...
              {experiences.map((exp) => (
                <li key={exp.id} className="relative flex items-start pr-16">
                  ...
                  <div className="ml-4 flex-1 min-w-0">
                    ...
                    <p className="text-sm text-gray-400 dark:text-zinc-500 mt-0.5">{formatDate(exp.startDate)} - {exp.isCurrent ? 'Present' : (exp.endDate ? formatDate(exp.endDate) : '')}</p>
                    {exp.description && <p className="mt-2 text-sm text-gray-500 dark:text-zinc-400 whitespace-pre-wrap">{exp.description}</p>}
                  </div>
                  {isOwner && (
                    <div className="absolute top-0 right-0 flex items-center -space-x-1">
                      <Button variant="ghost" size="sm" className="text-gray-600 dark:text-zinc-300 hover:text-primary-600 dark:hover:text-primary-400" onClick={() => openFormModal(exp)}><Edit className="h-4 w-4" /></Button>
                      <Button variant="ghost" size="sm" className="text-gray-600 dark:text-zinc-300 hover:text-red-600 dark:hover:text-red-400" onClick={() => openDeleteModal(exp)}><Trash2 className="h-4 w-4" /></Button>
                    </div>
                  )}
                </li>
              ))}
            ...
            <p className="text-sm text-center text-gray-500 dark:text-zinc-400 py-4">No work experience has been added yet.</p>
          )}
        </div>
      </div>
    </>
