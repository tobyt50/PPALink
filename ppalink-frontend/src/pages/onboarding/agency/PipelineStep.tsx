import { ArrowLeft, ArrowRight, BadgeCheck, Filter } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import { Button } from '../../../components/ui/Button';
import { OnboardingProgressBar } from '../candidate/SummaryStep';
import { motion } from 'framer-motion';

const DemoApplicantCard = ({ name, level, skills }: { name: string; level: string; skills: string[] }) => (
    <div className="rounded-xl bg-white dark:bg-zinc-900 p-4 shadow-sm dark:shadow-none dark:ring-1 dark:ring-white/10 ring-1 ring-gray-100">
        <div className="flex items-start">
            ...
            <div className="ml-3 flex-1 min-w-0">
                <p className="font-semibold text-sm text-gray-800 dark:text-zinc-100 truncate">{name}</p>
        ...
        <div className="mt-3 flex flex-wrap gap-1.5">
            {skills.map((skill) => (
                <span key={skill} className="rounded-full bg-green-50 dark:bg-green-950/60 px-2.5 py-1 text-xs font-medium text-green-700 dark:text-green-300">
                    {skill}
                </span>
            ))}
        </div>
...
const DemoPipelineColumn = ({ title, count, children }: { title: string; count: number; children?: React.ReactNode }) => (
        ...
        >
            ...
            <div className="mb-6 text-center">
                ...
                <p className="mt-2 text-gray-600 dark:text-zinc-300 max-w-lg mx-auto">
                    Our drag-and-drop pipeline makes it easy to track candidates. As applicants come in, you can move them through stages like "Reviewing", "Interview", and "Offer".
                </p>
            </div>
            ...
            <div className="my-8 p-4 bg-gray-50 dark:bg-zinc-800/50 rounded-xl border dark:border-zinc-800 select-none pointer-events-none">
                <div className="grid grid-cols-3 gap-3">
                    <DemoPipelineColumn title="Applied" count={1}>
                        <DemoApplicantCard name="Bello Akim" level="NYSC Verified" skills={['Java', 'SQL']} />
                    </DemoPipelineColumn>
                    <DemoPipelineColumn title="Interview" count={2}>
                        <DemoApplicantCard name="Chioma Nwosu" level="NYSC Verified" skills={['React', 'Figma']} />
                        <DemoApplicantCard name="David Okeowo" level="NYSC Verified" skills={['Python', 'Design']} />
                    </DemoPipelineColumn>
                    <DemoPipelineColumn title="Offer" count={1}>
                        <DemoApplicantCard name="Fatima Yusuf" level="NYSC Verified" skills={['Excel', 'Finance']} />
                    </DemoPipelineColumn>
                </div>
                <p className="text-xs text-center text-gray-500 dark:text-zinc-400 mt-4">This is a non-interactive demo. You can drag and drop candidates on your actual job pipeline page.</p>
            </div>

            <div className="mt-12 flex justify-between items-center pt-6 border-t border-gray-100 dark:border-zinc-800">
                <Button type="button" variant="outline" onClick={() => navigate('/onboarding/agency/post-job')}>
