...
const summarySchema = z.object({
  summary: z.string().min(30, 'Please write at least 30 characters.').max(1000, 'Summary cannot exceed 1000 characters.'),
});

type SummaryFormValues = z.infer<typeof summarySchema>;

export const OnboardingProgressBar = ({ step, totalSteps }: { step: number; totalSteps: number }) => (
    <div className="mb-8">
        <p className="text-sm text-center text-gray-500 dark:text-zinc-400 mb-2">Step {step} of {totalSteps}</p>
        <div className="w-full bg-gray-200 dark:bg-zinc-800 rounded-full h-2">
            <div className="bg-gradient-to-r from-primary-600 to-green-500 dark:from-primary-500 dark:to-green-400 h-2 rounded-full transition-all duration-500" style={{ width: `${(step / totalSteps) * 100}%` }}></div>
        </div>
    </div>
);
...
const SummaryStep = () => {
    ...
    const { register, handleSubmit, formState: { errors, isSubmitting } } = useForm<SummaryFormValues>({
    ...
    return (
        ...
        >
            ...
            <div className="text-center mb-6">
                ...
                <p className="mt-2 text-gray-600 dark:text-zinc-300">This is your elevator pitch to recruiters. Make it count!</p>
            </div>
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                <div className="space-y-1.5">
                    ...
                    <Textarea
                        ...
                        {...register('summary')}
                    />
                    {errors.summary && <p className="text-xs text-red-500 dark:text-red-400 mt-1">{errors.summary.message}</p>}
                </div>
