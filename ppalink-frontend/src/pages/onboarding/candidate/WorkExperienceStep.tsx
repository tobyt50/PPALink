...
} from "../../candidates/forms/WorkExperienceForm";
...
const WorkExperienceStep = () => {
  ...
  const [deleteModalState, setDeleteModalState] = useState<{
    isOpen: boolean;
    expId: string | null;
  }>({ isOpen: false, expId: null });

  const openModal = (exp?: WorkExperience) => {
    setEditingExp(exp || null);
    setIsModalOpen(true);
  };
  const openDeleteModal = (expId: string) =>
    setDeleteModalState({ isOpen: true, expId });
  const closeDeleteModal = () =>
    setDeleteModalState({ isOpen: false, expId: null });

  const handleDelete = async () => {
    if (!deleteModalState.expId) return;
    await toast.promise(
      candidateService.deleteWorkExperience(deleteModalState.expId),
      {
  ...
  return (
    <>
      ...
      <ConfirmationModal
        ...
        description="Are you sure you want to permanently remove this work experience record?"
      ...
      >
        ...
        <div className="text-center mb-6">
          ...
          <p className="mt-2 text-gray-600 dark:text-zinc-300">
            Detail your professional history, starting with the most recent.
          </p>
        </div>
        ...
        <div className="flex justify-between items-center pt-4 border-t border-gray-100 dark:border-zinc-800">
          <Button
            type="button"
            variant="outline"
            onClick={() => navigate("/onboarding/candidate/summary")}
          >
