import { Calendar, CheckCircle, ChevronLeft, Edit, Eye, EyeOff, Globe, Loader2, MapPin, Tag, Wallet } from 'lucide-react';
...
const AdminJobDetailsPage = () => {
  ...
  const [modalState, setModalState] = useState<{ isOpen: boolean; action: 'unpublish' | 'republish' | null }>({ isOpen: false, action: null });
  ...
  const handleModalConfirm = async () => {
    ...
    const actionPromise = isUnpublishing ? adminService.adminUnpublishJob(jobId) : adminService.adminRepublishJob(jobId);
    const successMessage = `Job has been ${isUnpublishing ? 'unpublished' : 'republished'}.`;
  ...
  return (
    <>
      <ConfirmationModal
        ...
        description={isPrivate ? "Are you sure? This will make the job visible to all candidates on the public job board." : "Are you sure? This will make the job invisible to candidates."}
      ...
      <div className="mx-auto max-w-5xl space-y-6">
        ...
        <div className="rounded-2xl bg-white dark:bg-zinc-900 shadow-md dark:shadow-none dark:ring-1 dark:ring-white/10 ring-1 ring-gray-100 overflow-hidden">
          <div className="p-6 border-b border-gray-100 dark:border-zinc-800 flex flex-col sm:flex-row justify-between items-start gap-4">
            <div>
              <h1 className="text-2xl md:text-3xl font-extrabold tracking-tight bg-gradient-to-r from-primary-600 dark:from-primary-500 to-green-500 dark:to-green-400 bg-clip-text text-transparent">{job.title}</h1>
              <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-gray-500 dark:text-zinc-400 mt-2">
                ...
                {agency?.cacVerified && (<span className="flex items-center text-xs font-semibold text-green-700 dark:text-green-300 bg-green-100 dark:bg-green-950/60 px-2 py-0.5 rounded-full"><CheckCircle className="h-3 w-3 mr-1"/>CAC Verified</span>)}
                {agency?.domainVerified && !agency.cacVerified && (<span className="flex items-center text-xs font-semibold text-blue-700 dark:text-blue-400 bg-blue-100 dark:bg-blue-950/60 px-2 py-0.5 rounded-full"><CheckCircle className="h-3 w-3 mr-1"/>Verified Domain</span>)}
              </div>
            </div>
            <div className="flex flex-shrink-0 gap-2">
              ...
              {isPrivate ? (
                  <Button size="sm" variant="primary" onClick={() => openModal('republish')}>
                    <Eye className="h-4 w-4 mr-2" />Republish
                  </Button>
              ) : (
                  <Button size="sm" variant="destructive" onClick={() => openModal('unpublish')}>
          ...
          <div className="p-6 grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="md:col-span-1 space-y-6">
              ...
              <DetailField
                ...
                label="Location"
              >
                {job.isRemote ? 'Remote' : locationState || 'Not specified'}
              </DetailField>
            </div>
